...
6. **Detect session recovery state**
   ...
   [ok] 1.1 ... (checked in tasks.md)
   WARNING: 1.2 ... (next task to resume)
...
- **strict**: Error — "[Skill check] subagent-driven-development X — 请安装 subagent 技能后重试". Halt. Do not proceed without subagent in strict mode. Run \`openspec init --tools claude\` to install bundled skills, or manually copy to \`.claude/skills/subagent-driven-development/\`.

- **enhanced**: Hard stop first — "[Skill check] subagent-driven-development X — subagent skill not found". Report the missing skill to the user with clear instructions to install it before proceeding. Only if the user explicitly confirms they cannot or will not install the skill, execute the task locally with the following subagent-equivalent discipline:
...
\`\`\`
WARNING: Sub-step compliance failure:
...
After marking, announce: "[x] Task X.Y complete (N/M done)"
