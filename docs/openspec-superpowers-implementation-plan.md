...
| **内容** | 支持 `[TDD]` / `[TDD: Lite]` / `[TDD: Skip]` 标注；可选 `[Spec: REQ-xxx]` 标注（实现 Section 2.6.1 的 spec 上下文精确注入）；Full TDD task 展开 RED→GREEN→REFACTOR 子 checklist |
...
| 检查项 | 预期 |
|--------|------|
| 模板中包含 `[TDD]` 标注语法说明 | 有，且说明对应 RED→GREEN→REFACTOR |
...
| 包含 `Skill({skill: "test-driven-development"})` 调用指令 | 有，且在 `[TDD]` 条件下 |
