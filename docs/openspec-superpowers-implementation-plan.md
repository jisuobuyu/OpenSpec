...
| Phase | 名称 | Tasks | 关键路径 | 依赖 |
|-------|------|-------|----------|------|
| **P0** | Schema & Config 基础设施 | 4 | 是 | — |
| **P1** | Apply 编排 + TDD 级别 + Simplify 集成 | 6 | 是 | P0 |
| **P2** | Verify 双层 + Review 编排 | 4 | 是 | P1 |
| **P3** | 生命周期完整性（abort/rewind/unarchive） | 5 | 否 | P0 |
| **P4** | 并行安全（冲突检测 + 依赖管理） | 5 | 否 | P0 |
| **P5** | 技能降级 + 会话恢复 + 度量 | 4 | 否 | P1, P2 |
| **P6** | 兼容性 + 测试 + 文档 | 3 | 是 | P0-P5 |

**总计**：31 tasks。P3/P4 可与 P1/P2 并行。
...
| **内容** | archive 前自动运行 verify（若未运行）、merge delta specs、清理 worktree |
| **验收** | `/opsx:archive` 完成 specs 合并 + worktree 清理 |
