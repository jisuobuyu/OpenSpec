...
**目标**：`specpower-driven` schema 可被系统加载，`config.yaml` 支持 `discipline` 配置，profile 支持 `enhanced`/`strict`。

> **无需注册步骤**：`schemas/specpower-driven/schema.yaml` 放入包根目录即自动被 `resolver.ts` 发现为 built-in schema。

### Task 0.1 — 新建 `specpower-driven` schema YAML + 模板目录

| 维度 | 内容 |
|------|------|
| **新建** | `schemas/specpower-driven/schema.yaml` |
| **内容** | 6 artifacts（proposal, exploration, specs, design, tasks, review）+ apply / verify / archive 三阶段（对标设计规格 Section 4） |
| **参照** | `schemas/spec-driven/schema.yaml`（line 1-154） |
| **新建** | `schemas/specpower-driven/templates/` 下 6 个模板文件：`proposal.md`, `explore.md`, `specs.md`, `design.md`, `tasks.md`, `review.md`（explore 和 review 为 optional artifact 模板） |
| **验收** | `openspec validate` 不报错；`openspec templates --schema specpower-driven` 列出 6 个模板路径 |
...
| **改动** | ① `ProjectConfigSchema` 新增可选 `discipline` 节点（`level`, `subagent.mode`, `worktree.enabled`, `exploration.search_history`）② `readProjectConfig()` 中追加 discipline 的 safeParse |
| **向后兼容** | 全部字段 optional + 默认值（level=strict, subagent=per-task, worktree=true, exploration.search_history=false） |
...
| **验收** | specpower-driven schema.yaml 通过 Zod 校验；含 `depends_on` 的 `.openspec.yaml` 解析正确 |

**P0 验收标准**：`openspec init --schema specpower-driven --profile enhanced` 创建项目 → 生成全部 skill/command 文件 → `openspec status` 正常显示 artifact 状态。
...
| 维度 | 内容 |
|------|------|
| **文件** | `schemas/specpower-driven/templates/tasks.md` |
| **内容** | 支持 6 步嵌入式 TDD 子步骤（RED/Verify RED/GREEN/Verify GREEN/REFACTOR/SIMPLIFY），每个 task 必须包含完整 TDD 周期子 checklist |
...
| **内容** | 外循环（task 迭代器）+ Pre-context / Skill 执行 / Post-checkpoint 三段式结构。关键指令：① 声明"你是 OpenSpec apply 外循环的 controller" ② 每次只处理一个 task ③ B1: TDD 周期（6 步嵌入式，强制执行）B2: `Skill({skill: "subagent-driven-development"})` 包裹 TDD（执行增强），不可用则降级为本地 B2-fallback（spec check + debugging + quality review）apply 开始时快照 `config.yaml` 的 discipline 配置，整个 change 使用一致配置（防热变更，对标设计规格 Section 8.5） |
...
| **内容** | SIMPLIFY 子步骤嵌入 task（`- [ ] SIMPLIFY: Review changed files`），apply 时本地执行。独立 `/opsx:simplify` 命令仍可 ad-hoc 调用。 |
...
### Task 1.5 — 编写 `explore.md` + `proposal` 模板（软依赖连接）

| 维度 | 内容 |
|------|------|
| **文件** | `schemas/specpower-driven/templates/explore.md`（新建）+ `schemas/specpower-driven/templates/proposal.md` |
| **内容** | explore 模板：灵活格式（Key Insights / Options / Recommendation / Open Questions 均可选）；proposal 模板：嵌入"若 explore.md 存在则读取其内容作为输入，否则使用对话历史"的条件读取逻辑 |
| **验收** | 存在 explore.md 时 proposal 指令自动引用其内容 |
...
| **改动** | 确认 ALL_WORKFLOWS 含 simplify；`openspec update` 生成所有 skill/command 文件 |
...
| **内容** | 嵌入分层融合模型：保持自由探索姿态；检测到多方案 trade-offs 时**提议**（非自动）调用 `Skill({skill: "brainstorming"})`；结束后提议生成 explore.md |
...
| **验收** | 现有 core 创建的 change 在 specpower-driven schema 下可正常 apply |
...
| 维度 | 内容 |
|------|------|
| **文件** | `README.md`（追加 specpower-driven schema 说明） + 已有 `docs/openspec-superpowers-fusion.md` |
...
| 文件 | Phase | 改动 | 风险 |
|------|-------|------|------|
| `schemas/specpower-driven/schema.yaml` | P0 | 新建 | 低 |
| `schemas/specpower-driven/templates/*.md` | P0-P1 | 新建 7 个 | 低 |
...
| `src/cli/index.ts` | P4-P5 | 追加命令（metrics, verify, check） | 低 |
| `src/commands/workflow/compliance-check.ts` | P5 | 新建 — C0 静态合规检查 | 低 |
...
| M | Phase | 可交付 | 估时 |
|---|-------|--------|------|
| M1 | P0 | `openspec init --schema specpower-driven` 可用 | 2d |
...
| 类别 | 文件 |
|------|------|
| Schema | `schemas/specpower-driven/schema.yaml` + 6 模板 |
| 编排模板 | apply, verify, review, simplify, ff, explore, abort, rewind, unarchive（9 个 workflow） |
| 核心逻辑 | consistency-auditor, conflict/detector, conflict/circular-deps, metrics/collector, skill-check |
| CLI | `openspec verify --change`, `openspec check --change`, `openspec metrics`, `openspec status --deps`, archive 冲突检测 |
...
```bash
# 1. 文件存在性
ls schemas/specpower-driven/schema.yaml
ls schemas/specpower-driven/templates/
# 预期: 列出 6 个 .md 文件

# 2. Schema 可被系统发现
openspec schemas
# 预期: 输出列表中包含 specpower-driven
...
openspec init --schema specpower-driven --profile enhanced --tools claude /tmp/test-project
...
openspec init --schema specpower-driven --profile strict --tools claude /tmp/test-strict
...
```bash
# 1. specpower-driven schema 通过 Zod 校验
...
```bash
openspec new change test-tdd-levels --schema specpower-driven /tmp/test-project
...
| 检查项 | 预期 |
|--------|------|
| 模板中包含 6 步 TDD 子步骤格式 | 有，RED/Verify RED/GREEN/Verify GREEN/REFACTOR/SIMPLIFY |
| 模板中包含 6 步 TDD 子步骤格式说明 | 有，每 task 必须嵌入 |
...
| TDD 6 步子步骤嵌入式执行 | 有，each task has sub-checkboxes |
...
| 包含 `Skill({skill: "subagent-driven-development"})` 调用 | 有，Per-Task 模式，不可用则降级 |
...
```bash
cat /tmp/test-project/openspec/schemas/specpower-driven/templates/explore.md
cat /tmp/test-project/openspec/schemas/specpower-driven/templates/proposal.md
```

| 检查项 | 预期 |
|--------|------|
| explore 模板：Key Insights / Options / Recommendation / Open Questions | 四节均标注为可选 |
| proposal 模板：条件读取 explore.md 的逻辑 | 有，"若 explore.md 存在则读取" |
...
# 预期: 输出 compliance check 结果，报告 TDD 子步骤完成情况
...
| 检查项 | 预期 |
|--------|------|
| `openspec metrics` 输出 6 项 | 1 项自动（specCoverage，由 `openspec verify --change` 写入）+ 5 项手动 |
| `openspec verify --change` 后自动记录 | metrics.yaml 中新增 specCoverage 快照 |
| `--json` 输出 | 合法 JSON |
...
# 2. 切换到 specpower-driven schema 后重新 apply
...
| 检查项 | 预期 |
|--------|------|
| 旧 tasks.md 在 specpower-driven 下正常 apply | TDD 级别自动推断，无报错 |
...
# 预期: 有 specpower-driven schema 说明段落
```
