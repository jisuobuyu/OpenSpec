...
| **新建** | `schemas/specpower-driven/templates/` 下 6 个模板文件：`proposal.md`, `exploration.md`, `specs.md`, `design.md`, `tasks.md`, `review.md`（exploration 和 review 为 optional artifact 模板） |
...
| **向后兼容** | 全部字段 optional + 默认值（level=strict, subagent=adaptive, worktree=true, exploration.search_history=false） |
...
| 维度 | 内容 |
|------|------|
| **文件** | `schemas/specpower-driven/templates/exploration.md`（新建）+ `schemas/specpower-driven/templates/proposal.md` |
...
```bash
cat /tmp/test-project/openspec/schemas/specpower-driven/templates/exploration.md
