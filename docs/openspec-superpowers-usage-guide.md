...
```markdown
- [ ] 1.1 [TDD] [Spec: REQ-auth-login] 实现邮箱登录接口
```
...
Phase A: Pre-context（OpenSpec 编排）
  ├─ 解析 [TDD] → 确定调用 test-driven-development 技能
...
| 命令 | 调用的技能 | 触发条件 |
|------|-----------|----------|
| `/opsx:apply` | `test-driven-development` | `[TDD]`  |
| `/opsx:apply` | `simplify` | 每个 task 完成后的 Post-checkpoint |
| `/opsx:apply` | `subagent-driven-development` | 每个 task 强制执行 |
...
Apply 模板在 Post-checkpoint 阶段的 C0 步骤会自动检查技能调用合规性——确保标注了 `[TDD]`  的任务确实调用了 `test-driven-development` 技能。

**检查机制**（在 Post-checkpoint C0 中执行）：

| 检查规则 | 触发条件 | 行为 |
|----------|----------|------|
| `[TDD]` 任务 → 是否调用了 `Skill({skill: "test-driven-development"})`？ | 任务完成时 | 未调用 → 提示 Retry/Explain/Override |
...
- [ ] 2.1 [TDD] [Spec: REQ-oauth-google] 实现 Google OAuth 回调
- [ ] 2.2 [TDD] [Spec: REQ-oauth-github] 实现 GitHub OAuth 回调
```

| 标注 | 作用 |
|------|------|
| `[TDD]` | 完整 RED→GREEN→REFACTOR 循环 |
...
| `discipline.level` | `strict` | `core` / `enhanced` / `strict` | 工程纪律级别。TDD 强制执行，所有级别均调用 TDD 技能 |
...
| TDD 强制执行 | 所有 task 必须标注 `[TDD]`，AI 对每个 task 调用 `Skill("test-driven-development")` |
...
# AI 自动：
#   Task 1.1 [TDD] → Skill("test-driven-development")
#   → git commit -m "simplify(1.1): ..."
...
# AI 行为：
#   每个 [TDD] task → Skill("test-driven-development")
#   技能缺失 → 报错（不降级）
#   不允许 Skip TDD
```
