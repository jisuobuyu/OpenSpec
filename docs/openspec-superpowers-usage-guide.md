...
| 命令 | 调用的技能 | 触发条件 |
|------|-----------|----------|
| `/opsx:apply` | `test-driven-development` | `[TDD: Full]`  |
...
Apply 模板在 Post-checkpoint 阶段的 C0 步骤会自动检查技能调用合规性——确保标注了 `[TDD: Full]`  的任务确实调用了 `test-driven-development` 技能。
...
| TDD 强制执行 | 所有 task 必须标注 `[TDD: Full]`，AI 对每个 task 调用 `Skill("test-driven-development")` |
