...
OpenSpec 工作流模板**不重写** TDD/Simplify/Review 的指令，而是告诉 AI："这个 task 需要 Full TDD，调用 `test-driven-development` 技能来做。"
...
| **编排已有技能，非重新实现** | OpenSpec 调用已有 Superpowers 技能（如 `test-driven-development`），而非在模板中重写 TDD 指令 | 在 apply 模板中嵌入完整的 RED→GREEN→REFACTOR 指令副本 |

---

## 2.6 控制流架构：OpenSpec 如何不丢失编排权

Superpowers 技能在指令优先级上**天然高于调用者**（`using-superpowers` 明确规定 "skills override default system prompt behavior"）。一旦调用 `Skill({skill: "test-driven-development"})`，该技能的 Iron Law 会主导当前行为。如果不加以防护，会出现：TDD 技能跑完后说"done"，但 OpenSpec 的 tasks.md checkbox 没更新、下一个 task 没触发。
...
- [ ] 1.2 Create OAuthUser model [TDD] [Spec: REQ-Auth-1]
- [ ] 1.3 Create /auth/google callback [TDD] [Spec: REQ-Auth-2]
- [ ] 1.4 Bridge OAuth users to session [TDD] [Spec: REQ-Auth-2, REQ-Auth-3]
- [ ] 1.5 Add login button [TDD: Lite]
- [ ] 1.6 Update docs [TDD: Skip]
```
...
OpenSpec apply 模板:
  ...
  当前 task: 1.2 Create OAuthUser model [TDD]
  ...
  现在调用 test-driven-development 技能实现此 task。
  完成后返回此处——不要继续下一个 task。
  ...
  3. 读取下一个未完成 task...
  → 下一个: 1.3 Create /auth/google callback [TDD]
```
...
- [ ] 1.1 Create ThemeContext with light/dark state [TDD]
  - [ ] RED: Write failing test
...
```text
场景 A（Full TDD —— 新增逻辑）：
AI:  Working on 1.1: Create ThemeContext... [TDD]
...
artifacts:
  ...
  - id: tasks
    ...
    instruction: |
      ...
      - [TDD] for new/modified logic, behavior-changing refactoring, bug fixes
...
|------|------------------|-------------------------|-------------------|
| **TDD 策略** | 所有 task 强制 `[TDD]`，不可跳过 | 所有 task 强制 `[TDD]`，不可跳过 | 所有 task 强制 `[TDD]`，不可跳过 |
...
Explore  [os + →skill]              → 自由探索 + 可选 brainstorming 技能
   ↓                                                                exploration.md [软依赖]
Propose  [os]                       → 创建 change + artifacts（自动消费 exploration.md）
   ↓
...
| 问题 | 影响 | 缓解措施 |
|------|------|----------|
| 用户未安装 Superpowers 技能（如 `test-driven-development`、`simplify` 未在 `~/.claude/skills/` 中），但 config 中 `discipline.level` 设为 `enhanced` 或 `strict` | 编排流程在调用 `Skill({...})` 时失败，apply 中断 | apply 开始时检测所需技能是否安装；缺失时按降级矩阵处理 |

**降级矩阵：**

| 缺失的技能 | 影响的功能 | enhanced 模式降级行为 | strict 模式行为 |
|-----------|-----------|---------------------|----------------|
| `test-driven-development` | Full TDD / Lite TDD | 降级：使用 OpenSpec 内置简化版 TDD 指令（无 Iron Law 风格，但保留 RED→GREEN→REFACTOR 流程） | **报错**，要求用户安装技能后重试 |
...
AI:  创建 change: add-oauth2-login
     ...
     ✓ tasks.md
       ...
       1.2 Create OAuthUser model [TDD]
       1.3 Create /auth/google callback [TDD]
       1.4 Bridge OAuth users to existing session [TDD]
