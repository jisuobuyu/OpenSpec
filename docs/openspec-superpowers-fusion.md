...
- [ ] 1.2 Create OAuthUser model [TDD: Full] [Spec: REQ-Auth-1]
- [ ] 1.3 Create /auth/google callback [TDD: Full] [Spec: REQ-Auth-2]
- [ ] 1.4 Bridge OAuth users to session [TDD: Full] [Spec: REQ-Auth-2, REQ-Auth-3]
...
OpenSpec apply 模板:
  ...
  当前 task: 1.2 Create OAuthUser model [TDD: Full]
  ...
  3. 读取下一个未完成 task...
  → 下一个: 1.3 Create /auth/google callback [TDD: Full]
```
...
> **检测时机**：`/opsx:apply` 执行前自动运行，输出类似 `[Skill check] test-driven-development ✓ / subagent-driven-development ✗ (降级为 Inline)` 的状态面板。`core` 模式不调用任何 Superpowers 技能，因此不受影响。
