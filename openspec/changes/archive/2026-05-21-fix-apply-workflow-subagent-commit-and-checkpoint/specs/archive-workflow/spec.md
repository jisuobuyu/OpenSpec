...
### Requirement: Single Commit Created at Archive Time

The archive workflow SHALL create a single git commit containing all implementation changes. The commit message SHALL list every completed task by ID and description, and SHALL include verification and review status.
...
- **THEN** the controller SHALL create a commit with the format:
  ```
  ...
  ```
- **AND** the commit SHALL be created before the directory move to archive
...
- **WHEN** the archive workflow reaches the commit step
- **AND** `openspec instructions apply --json` reports `progress.remaining > 0`
- **THEN** the controller SHALL display: "N task(s) still unmarked in tasks.md. Mark them first before commit."
- **AND** the controller SHALL NOT proceed to `git commit`

#### Scenario: All tasks marked — commit proceeds

- **WHEN** the archive workflow reaches the commit step
- **AND** `openspec instructions apply --json` reports `progress.remaining === 0`
- **THEN** the controller SHALL proceed to create the commit

### Requirement: Archive Blocked Without Verify Pass

The archive workflow SHALL refuse to create a commit and move the change directory if verification was attempted but failed.
